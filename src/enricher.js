...
function scoreEmail(email, domain) {
    ...
    // Dental-specific scheduling / decision-maker prefixes
    if (/^(appointments?|scheduling|newpatient|new\.patient|booking)/i.test(prefix)) score += 10;
    if (/^(frontdesk|reception|office|admin)/i.test(prefix)) score += 8;
    if (/^(dr\.?|doctor|dentist)/i.test(prefix))             score += 7;
    if (/^(info|contact|hello|hi|care)/i.test(prefix))       score += 6;
